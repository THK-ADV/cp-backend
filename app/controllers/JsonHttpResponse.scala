...
trait JsonHttpResponse[A] {
  ...
  private def recover400(f: Future[Result])(implicit
      writes: Writes[A]
  ): Future[Result] =
    f.recover { case NonFatal(t) => badRequest(t) }

  def okSeq(f: Future[Seq[A]])(implicit writes: Writes[A]): Future[Result] =
    recover400(f.map(s => Ok(Json.toJson(s))))

  def ok(f: Future[A])(implicit writes: Writes[A]): Future[Result] =
    recover400(f.map(s => Ok(Json.toJson(s))))

  def okOpt(f: Future[Option[A]])(implicit writes: Writes[A]): Future[Result] =
    recover400(
      f.map(s => s.fold(NotFound(Json.obj()))(s => Ok(Json.toJson(s))))
    )

  def created(f: Future[A])(implicit writes: Writes[A]): Future[Result] =
